...
if (historyList) {
  historyList.innerHTML = scans.map(scan => `
    <div class="history-item">
      ...
      ${scan.image ? `
        <img class="history-thumb" src="${scan.image}" alt="Meal scan" />
      ` : ""}

      <div>
        <strong>${scan.mealName}</strong>

        <div class="history-meta">
          ${scan.calories} Calories · ${displayGoal[scan.goal] || scan.goal}
        </div>

        <div class="history-meta">
          ${scan.confidence ? `${scan.confidence} confidence · ` : ""}${scan.createdAt || ""}
        </div>
      </div>
